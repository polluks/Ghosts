  ...
  case "$opt" in
    t)
      ...
      if [ -d ${OPTARG} ]; then
        ...
        # by standard all possible z5/multi-z targets are enabled, all z3-only targets are disabled (see below)
        ...
        # these targets only support z3 and are deactivated by default, add them to the zip line above if desired
