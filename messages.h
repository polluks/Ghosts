...
MSG_RUB_DEFAULT, MSG_SQUEEZE_DEFAULT:
	if (action == ##Rub) print_ret "You make a half-hearted attempt at cleaning ", (the) noun, ".";
	"You give ", (the) noun, " a bit of a squeeze.";
#Endif;
...
#IfTrue MSG_TAKE_PART_OF < 1000;
	MSG_TAKE_PART_OF:
		...
		print_ret (The) p_arg_1, " seems to be a part of ", (the) p_arg_2, ".";
#EndIf;
#Ifndef OPTIONAL_NO_DARKNESS;
...
#Iftrue MSG_SCORE_DEFAULT < 1000;
	MSG_SCORE_DEFAULT:
#Ifdef NO_SCORE;
		"This game has no score.";
#Ifnot;
		if (deadflag) print "You"; else print "So far you";
		print " played ", turns, " turn";
		if(turns ~= 1) print "s";
		print " and scored ", score, " out of ", MAX_SCORE, " points";
		rtrue;
#Endif;
#Endif;
#IfDef OPTIONAL_FULL_SCORE;
...
#IfTrue MSG_RESTART_RESTORE_OR_QUIT < 1000;
	MSG_RESTART_RESTORE_OR_QUIT:
		print "^Would you like to RESTART from the beginning, RESTORE a saved position";
#Ifdef OPTIONAL_PROVIDE_UNDO_FINAL;
...
#Endif;
#IfDef OPTIONAL_FULL_SCORE;
		print ", check your FULL score";
#EndIf;
		if(deadflag == 2 && AMUSING_PROVIDED == 0) print ", see a list of AMUSING things to do";
		print " or QUIT? ";
		rtrue;
#EndIf;
...
#IfTrue MSG_INSERT_NOT_CONTAINER < 1000;
	MSG_INSERT_NOT_CONTAINER:
		...
		print_ret (The) p_arg_1, " either can't contain any things or you shouldn't worry about it.";
#Endif;
...
#IfTrue MSG_RESTART_CONFIRM < 1000;
	MSG_RESTART_CONFIRM:
		print "Restart the game from the beginning? ";
#Endif;
#Ifndef NO_SCORE;
...
#IfTrue MSG_EMPTY_ALREADY_EMPTY < 1000;
	MSG_EMPTY_ALREADY_EMPTY:
		...
		print_ret (The) p_arg_1, " ", (isorare) p_arg_1, " already empty.";
#EndIf;
...
#IfTrue MSG_WAVE_DEFAULT < 1000;
	MSG_WAVE_DEFAULT:
		"You wave ", (the) noun, " in the air, with no apparent consequences.";
#EndIf;
#EndIf;
